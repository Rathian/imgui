...
function StartImgui( element, serveruri, targetwidth, targetheight, compressed ) {
    ...
    function websocketConnect( serveruri ) {
        ...
        websocket.onopen = function( evt ) {
            ...
            connecting = false;
            connected = true;
            websocket.send("ImInit=" + targetwidth + "," + targetheight);
            gclips.length = 0;
            onRender();
        };
        ...
        websocket.onmessage = function( evt ) {
            if( typeof evt.data == "string" ) {
                ...
                else if( evt.data.substring(0, 12) == "ImClipboard=" )
                {
					if(!copyTextToClipboard(evt.data.slice(12)))
						clipboard = evt.data.slice(12);
                }
				else
    ...
    function onWindowResize() {
        ...
        camera.position.z = 1;
		websocket.send("ImResize=" + width + "," + height);
    }
    ...
    function onMouseMove( event ) {
        ...
        if( camera_drag ) {
            ...
            if( clientactive )
                websocket.send("ImMouseMove=" + x + "," + y + "," + mouse_left + "," + mouse_right + "," + mouse_middle);
        }
    }
    ...
    function onMouseDown( event ) {
		...
		if( event.button == 1 ) mouse_middle = 1;
        ...
        if( camera_drag ) {
            ...
            if( clientactive )
            {
                websocket.send("ImMousePress=" + mouse_left + "," + mouse_right + "," + mouse_middle);
            }
        }
    }    
    ...
    function onMouseUp( event ) {
		...
		if( event.button == 1 ) mouse_middle = 0;
    ...
    function onKeyUp( event ) {
        ...
        if( event.which != 0 )
        {
            if(osxCommandKey || isSpecialKey(event))
            {
                ...
                {
					var isCtrl =  (osxCommandKey || event.ctrlKey)?1:0;	
					if (isCtrl && clipboard)// && (event.which == 80) && clipboard)
					{
