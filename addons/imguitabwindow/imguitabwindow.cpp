...
#   endif //CACHE_LAST_GRADIENT
    ...
    inline static ImU32 ColorMergeWithAlpha(ImU32 c, float alphaMult) {
        c &= 0x00FFFFFF;
        c |= ((ImU32)IM_F32_TO_INT8_SAT(alphaMult)) << IM_COL32_A_SHIFT;
        return c;
    }
...
#include "../imguihelper/imguihelper.h"
    ...
    static bool TabButton(const char *label, bool selected, bool *pCloseButtonPressedOut = NULL, const char* textOverrideIn = NULL, ImVec2 *pJustReturnItsSizeHereOut = NULL, const TabLabelStyle* pOptionalStyleToUseIn = NULL, ImFont *fontOverride = NULL, ImVec2 *pOptionalJustDrawTabButtonGraphicsUnderMouseWithThisOffset = NULL, ImDrawList *drawListOverride = NULL, bool privateReuseLastCalculatedLabelSizeDoNotUse = false, bool forceActiveColorLook = false) {
        ...
        if (hasCloseButton) {
            const ImU32 c = (held && btnHovered) ? tabStyle.colors[TabLabelStyle::Col_TabLabelCloseButtonActive] : btnHovered ? tabStyle.colors[TabLabelStyle::Col_TabLabelCloseButtonHovered] : 0;
            if (btnHovered) DrawListHelper::ImDrawListAddRect(drawListOverride, startBtn, endBtn, c, tabStyle.colors[TabLabelStyle::Col_TabLabelCloseButtonBorder], tabStyle.closeButtonRounding, 0x0F, tabStyle.closeButtonBorderWidth);
    ...
    void TabWindow::render()
    {
        ...
        if (g.HoveredRootWindow == ImGui::GetCurrentWindow())
        {
            ...
            if (dd.draggingTabSrc) {
                ...
                const ImVec2& mp = ImGui::GetIO().MousePos;
                const ImVec2 wp = dd.draggingTabImGuiWindowSrc->Pos;
                ImDrawList* drawList = ImGui::GetOverlayDrawList();    // This draws the dragging tab under the other tabs, and has OTHER problems with: e.g.: ImGui::GetStyle().Colors[ImGuiCol_ChildWindowBg]=ImVec4(0.4,0.4,0.4,1);ImGui::GetStyle().Alpha = 1.f;
