...
void NodeGraphEditor::drawPopup(ImVec2 scene_pos)
{
	char buffer[256];
	char prefix[256];
	int offsets[8];
	...
	for (int nt = 0, ntSize = getNumAvailableNodeTypes(); nt < ntSize; nt++) {
		...
		const char* name = pNodeTypeNames[ni.type];
		strcpy(buffer, name);

		if (depth > 0) {
			int d = depthmatch(prefix, name, name);
			while (d < depth) {
				if (d < rd) {
					ImGui::EndMenu();
					rd--;
		...
		if (depth == rd) {
			while (true) {
				...
				if (part == NULL) {
					...
					offsets[depth] = part - name + 1;
					offset += part - name + 1;
					buffer[offset - 1] = '\0';
					name = buffer + offset;
					memcpy(prefix, pNodeTypeNames[ni.type], offset);
					depth++;
...
#                   endif // SKIP_VERTICAL_GRADIENT
                ...
                for (int slot_idx = 0; slot_idx < node->InputsCount; slot_idx++)    {
					...
					ImU32 color = node->InputColors[slot_idx].second ? node->InputColors[slot_idx].first : style.color_node_input_slots;
                    ImGui::NGE_Draw::ImDrawListAddCircle(draw_list,connectorScreenPos,NODE_SLOT_RADIUS,color,style.color_node_input_slots_border,style.node_slots_num_segments,connectorBorderThickness);
                        ...
                        if ((deltaPos.x*deltaPos.x)+(deltaPos.y*deltaPos.y)<NODE_SLOT_RADIUS_SQUARED)   {
                            if (mustDeleteLinkIfSlotIsHovered)  {
								...
								if (dragNode.inputSlotIdx != -1) {
									...
									if (isNodeConnectableFrom(dragNode.node, dragNode.inputSlotIdx, node, slot_idx)) {
                ...
                for (int slot_idx = 0; slot_idx < node->OutputsCount; slot_idx++)   {
					...
					ImU32 color = node->OutputColors[slot_idx].second ? node->OutputColors[slot_idx].first : style.color_node_output_slots;
                    ImGui::NGE_Draw::ImDrawListAddCircle(draw_list,connectorScreenPos,NODE_SLOT_RADIUS,color,style.color_node_output_slots_border,style.node_slots_num_segments,connectorBorderThickness);
                        ...
                        if ((deltaPos.x*deltaPos.x)+(deltaPos.y*deltaPos.y)<NODE_SLOT_RADIUS_SQUARED)   {
                            if (mustDeleteLinkIfSlotIsHovered)  {
								...
								if (dragNode.outputSlotIdx != -1) {
									...
									if (isNodeConnectableFrom(node, slot_idx, dragNode.node, dragNode.outputSlotIdx)) {
            ...
            draw_list->ChannelsSetCurrent(0); // Deeper than Background
            {
                ...
                for (int link_idx = 0; link_idx < links.Size; link_idx++)
                {
					...
					ImU32 color = link.LinkColor.second ? link.LinkColor.first : style.color_link;
                    draw_list->AddBezierCurve(p1,cp1,cp2,p2,color,(nearestLinkId!=link_idx) ? link_line_width : (link_line_width*2.f), style.link_num_segments);
                }
...
bool NodeGraphEditor::isNodeConnectableFrom(const Node* inputNode, int input_slot, const Node* outputNode, int output_slot) const
{
	if (inputNode->OutputColors[input_slot].second &&
		outputNode->InputColors[output_slot].second)
	{
		return (inputNode->OutputColors[input_slot].first == outputNode->InputColors[output_slot].first);
	}
	else
	{
		return true;
...
bool FieldInfo::render(int nodeWidth)   {
    ...
    case FT_STRING: {
		...
		if (/*f.tooltip &&*/ f.tooltip[0]!='\0' && ImGui::IsItemHovered()) NodeGraphEditorSetTooltip(f.tooltip);
				...
				//    widgetIndex = 1;
				//}
				const bool browseButtonPressed = ImGui::Button("...##DummyLabelInputTextBrowseButton");
				static ImGuiFs::Dialog dlg;
				const char* chosenPath = dlg.chooseFileDialog(browseButtonPressed);
				if (strlen(chosenPath)>0) {
					strcpy(txtField, dlg.getChosenPath());
					changed = true;
					widgetIndex = 1;
				}
                ImGui::PopItemWidth();
