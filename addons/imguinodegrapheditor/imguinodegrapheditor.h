...
#   endif //IMGUINODE_MAX_SLOT_NAME_LENGTH
	...
	std::pair<ImU32, bool>	InputColors[IMGUINODE_MAX_INPUT_SLOTS];
	std::pair<ImU32, bool>	OutputColors[IMGUINODE_MAX_INPUT_SLOTS];
	...
	inline void SetInputSlotColor(int slot_no, ImU32 color, bool active = true) { InputColors[slot_no] = { color, active }; };
	inline void SetOutputSlotColor(int slot_no, ImU32 color, bool active = true) { OutputColors[slot_no] = { color, active }; };
...
struct NodeLink
{
	...
	NodeLink(Node* input_node, int input_slot, Node* output_node, int output_slot, ImU32 color)
		: InputNode(input_node)
		, InputSlot(input_slot)
		, OutputNode(output_node)
		, OutputSlot(output_slot)
		, LinkColor{ color, true }
	{ };
...
#       endif //NO_IMGUIHELPER_SERIALIZATION
    ...
    mutable void* user_ptr;
    static Style& GetStyle() {static Style styl;return styl;}
...
#   endif //IMGUITABWINDOW_H_
    ...
    bool addLink(Node* inputNode, int input_slot, Node* outputNode, int output_slot,bool checkIfAlreadyPresent = false)	{
        ...
        if (insert) {
			if (inputNode->OutputColors[input_slot].second)
				links.push_back(NodeLink(inputNode, input_slot, outputNode, output_slot, inputNode->OutputColors[input_slot].first));
			else
	...
	IMGUI_API bool isNodeConnectableFrom(const Node* inputNode, int input_slot, const Node* outputNode, int output_slot) const;
...
//--------------------------------------------------------------------------------
	...
	void drawPopup(ImVec2 scene_pos);
